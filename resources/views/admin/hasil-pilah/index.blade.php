...
@section('title', 'Hasil Pilah Sampah')
@section('page-title', 'Hasil Pilah Sampah')

@section('content')
{{-- Data jenis sampah untuk JavaScript --}}
<script>
    window._jenisSampahData = @json($jenisList);
</script>

<div x-data="hasilPilah()">

<div class="flex items-center justify-between mb-5">
    <p class="text-sm text-gray-500">Input dan kelola data hasil pilah sampah</p>
    <a href="{{ route('admin.hasil-pilah.create') }}" class="btn-primary">
...
<div class="card p-4 mb-5">
    <form method="GET" class="flex flex-wrap gap-3 items-end">
        ...
        <div>
            ...
            <select name="jenis_id" class="form-input w-40" onchange="this.form.submit()">
                <option value="">Semua Jenis</option>
                @foreach($jenisList as $j)
                    <option value="{{ $j->id }}" {{ $jenis == $j->id ? 'selected' : '' }}>{{ $j->nama }}</option>
                @endforeach
            </select>
        </div>
        <a href="{{ route('admin.hasil-pilah.index') }}" class="btn-secondary">Reset</a>
    </form>
</div>
...
<div class="grid grid-cols-2 gap-4 mb-5">
    <div class="card p-4 flex items-center gap-4">
        ...
        <div>
            <p class="text-xs text-gray-500">Total Berat Pilah</p>
            <p class="text-xl font-bold text-gray-900">{{ number_format($totalBerat, 2, ',', '.') }} kg</p>
        </div>
    </div>
    <div class="card p-4 flex items-center gap-4">
...
<div class="card overflow-hidden">
    <table class="tbl">
        <thead>
            <tr>
                <th>No</th><th>Tanggal</th><th>Jenis</th><th>Nama Sampah</th>
                <th>Berat (kg)</th><th>Harga/kg</th><th>Total Nilai</th><th>Aksi</th>
            </tr>
        </thead>
        <tbody>
        @forelse($data as $i => $item)
            <tr>
                ...
                <td class="font-medium">{{ $item->namaSampah?->nama ?? '—' }}</td>
                <td class="font-bold text-blue-700">{{ number_format($item->berat_kg, 2, ',', '.') }}</td>
                <td class="text-gray-500 text-sm">Rp {{ number_format($item->harga_per_kg, 0, ',', '.') }}</td>
                ...
                <td>
                    <div class="flex items-center gap-2">
                        ...
                        <form method="POST" action="{{ route('admin.hasil-pilah.destroy', $item) }}" class="inline">
                            @csrf @method('DELETE')
        ...
        @empty
            <tr><td colspan="8" class="text-center py-12 text-gray-400">
                <div class="text-4xl mb-2">📭</div>Belum ada data
            </td></tr>
        @endforelse
        </tbody>
    </table>
...
{{-- ============================================================
     MODAL EDIT — hanya aktif saat showModal && editMode
     ============================================================ --}}
<div x-show="showModal && editMode" x-cloak
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6"
        ...
        <div class="flex justify-between items-center mb-5">
            <h3 class="text-base font-bold">Edit Hasil Pilah</h3>
            <button @click="showModal = false" class="text-gray-400 hover:text-gray-600">✕</button>
        </div>
        <form method="POST" :action="'/admin/hasil-pilah/' + editForm.id" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
                <label class="form-label">Tanggal *</label>
                <input type="date" name="tanggal" x-model="editForm.tanggal" class="form-input" required>
            </div>
            <div>
                <label class="form-label">Jenis Sampah</label>
                <select x-model="editForm.jenis_id" @change="onJenisChangeEdit(editForm.jenis_id)" class="form-input">
                    <option value="">-- Pilih Jenis --</option>
                    <template x-for="j in jenisList" :key="j.id">
                        <option :value="j.id" x-text="j.nama" :selected="j.id == editForm.jenis_id"></option>
                    </template>
                </select>
            </div>
            <div>
                <label class="form-label">Nama Sampah *</label>
                <select name="nama_sampah_id" x-model="editForm.nama_sampah_id" class="form-input" required>
                    <option value="">-- Pilih Nama --</option>
                    <template x-for="n in editNamaList" :key="n.id">
                        <option :value="n.id" x-text="n.nama"
                                :selected="n.id == editForm.nama_sampah_id"></option>
                    </template>
                </select>
            </div>
            <div>
                <label class="form-label">Berat (kg) *</label>
                <input type="number" name="berat_kg" x-model="editForm.berat_kg"
...
document.addEventListener('alpine:init', () => {
    Alpine.data('hasilPilah', () => ({
        ...
        openEdit(data) {
            ...
            this.editNamaList = [];
            if (this.editForm.jenis_id) this.onJenisChangeEdit(this.editForm.jenis_id);
            this.showModal = true;
        },



        // Jenis/nama untuk EDIT
        async onJenisChangeEdit(jenisId) {
            this.editForm.nama_sampah_id = '';
            this.editNamaList = [];
            if (!jenisId) return;
            const res = await fetch('/admin/api/nama-sampah?jenis_id=' + jenisId);
            this.editNamaList = await res.json();
        }
    }));
});
</script>
